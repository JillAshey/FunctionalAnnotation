# Title: Plob Annotation Compilation
# Author: Jill Ashey
# Date: 12/14/2020



## Species: P. lobata (using the closely related Plutea genomic information, as Plobata has no genomic info)

#This script takes the results of functional annotation services and combines the results. Nucleotide CDS sequences were annotated using DIAMONDSEARCH BLASTX, resulting in xx hits. These hits were used as input into:
#  - Uniprot
#  - Blast2GO
...
plob_blast <- read_tsv("~/Desktop/PutnamLab/Repositories/FunctionalAnnotation/FunctionalAnnotation/Diamond/Plut_annot.tab", col_names = FALSE)
colnames(plob_blast) <- c("seqName", "top_hit", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore",  "qlen", "slen")
head(plob_blast)
dim(plob_blast) #4315 x 14
...
#Because there were many Diamond BLAST hits for Plob, I had to break up the tab file into chunks that Uniprot could handle. So there are 2 pdam_Uniprot files to be read and then rbind() together
u1 <- read_tsv("~/Desktop/PutnamLab/Repositories/FunctionalAnnotation/FunctionalAnnotation/Uniprot/plob_Uniprot_1.tab", col_names = TRUE)
...
u2 <- read_tsv("~/Desktop/PutnamLab/Repositories/FunctionalAnnotation/FunctionalAnnotation/Uniprot/plob_Uniprot_2.tab", col_names = TRUE)
...
nrow(uniprot_GO) # 717 total GO terms from Uniprot
length(unique(uniprot_GO$GO.ID)) # 246 unique GO terms from Uniprot
length(unique(uniprot_GO$gene_id)) # 298 unique gene ids from Uniprot 
...
B2G_results <- read.csv("~/Desktop/PutnamLab/Repositories/FunctionalAnnotation/FunctionalAnnotation/Blast2GO/plut_blast2go_table.csv")
colnames(B2G_results)
...
head(B2G_results_GO)
nrow(B2G_results_GO) # 997 total GO terms from B2G
length(unique(B2G_results_GO$GO.ID)) # 288 unique GO terms from B2G
length(unique(B2G_results_GO$gene_id)) # 310 unique gene ids from B2G
...
plob_annot <- left_join(plob_blast, B2G_results, by="seqName")
plob_annot <- select(plob_annot, seqName, top_hit.x, length.x, evalue, bitscore, simMean, GO.ID, GO_names)
colnames(plob_annot) <- c("gene_id", "top_hit", "length", "evalue", "bitscore", "simMean", "GO.ID", "GO_names")
uniprot_results <- select(uniprot_results, -top_hit)
uniprot_results <- rename(uniprot_results, "top_hit"="my_list")
plob_annot <- merge(plob_annot, uniprot_results, by="top_hit", all.x = T)
plob_annot$GO.IDs <- paste(plob_annot$GO.ID, plob_annot$go_ids, sep=';') #generate new column with concatenated GO IDs
plob_annot$GO_terms <- paste(plob_annot$GO_names, plob_annot$gene_ontology, sep=';') #generate new column with concatenated GO IDs
plob_annot <- select(plob_annot, c("top_hit", "gene_id", "length.x", "evalue", "bitscore", "simMean", "uniprotkb_entry", "status", "protein_names", "gene_names", "organism", "ko", "kegg", "GO.IDs", "GO_terms"))
plob_annot <- rename(plob_annot, "GO.ID"="GO.IDs")
plob_annot <- rename(plob_annot, "length"="length.x")
names(plob_annot)
head(plob_annot)
tail(plob_annot)
dim(plob_annot) # 4315 x 15
write.csv(plob_annot, "~/Desktop/plob_FuncAnn_UniP_B2G.csv")
...
# IPS
IPS_GO <- read.csv("~/Desktop/PutnamLab/Repositories/SedimentStress/SedimentStress/Output/GOseq/plob_GOterms.csv", header=TRUE)
...
length(unique(IPS_GO$gene_id)) # 15559
...
head(IPS_GO)
nrow(IPS_GO) # 37062 total GO terms from IPS
length(unique(IPS_GO$GO.ID)) # 1994 unique GO terms from IPS
length(unique(IPS_GO$gene_id)) # 15559 unique gene ids from IPS

# From B2G and Uniprot
FuncAnn_UniP_B2G <- read.csv("~/Desktop/PutnamLab/Repositories/FunctionalAnnotation/FunctionalAnnotation/final_Annotations/plob_FuncAnn_UniP_B2G.csv")
length(unique(FuncAnn_UniP_B2G$gene_id)) # 4315
FuncAnn_UniP_B2G_GO <- filter(FuncAnn_UniP_B2G, grepl("GO",GO.ID)) #Select only gnes with GO terms
length(unique(FuncAnn_UniP_B2G_GO$gene_id)) # 341
...
nrow(FuncAnn_UniP_B2G_GO) # 1268 total GO terms from B2G + Uniprot
length(unique(FuncAnn_UniP_B2G_GO$GO.ID)) # 340 unique GO terms from B2G + Uniprot
length(unique(FuncAnn_UniP_B2G_GO$gene_id)) # 341 unique gene ids from B2G + Uniprot
...
length(unique(FIunique_gene)) # 147 gene ids unique to B2G + Uniprot

# Difference in gene ids for B2G + Uniprot and IPS - IPS
IFunique_gene <- setdiff(IPS_GO$gene_id, FuncAnn_UniP_B2G_GO$gene_id)
length(unique(IFunique_gene)) # 15365 gene ids unique to IPS
...
write.csv(full_annot, file = "~/Desktop/PutnamLab/Repositories/FunctionalAnnotation/FunctionalAnnotation/final_Annotations/plob_fullAnnot.csv")



## InterProScan
#IPS <- read.csv("~/Desktop/PutnamLab/FunctionalAnnotation/InterProScan/plob.interpro.gff3", header = F, sep = "\t")
...
# IPS_GO <- unique(IPS_GO)
# head(IPS_GO)
# nrow(IPS_GO) # 37072 total GO terms from IPS
# length(unique(IPS_GO$GO)) # 1995 unique GO terms from IPS
# length(unique(IPS_GO$gene_id)) # 15568 unique gene ids from IPS
# 
# 
# 
...
# length(unique(UI_GO)) # 206 similar GO terms between Uniprot and IPS
# 
# # Difference in GO terms for Uniprot and IPS - Uniprot
# UIunique_GO <- setdiff(uniprot_GO$GO.ID, IPS_GO$GO.ID)
# length(unique(UIunique_GO)) # 40 GO terms unique to Uniprot
# 
# # Difference in GO terms for B2G and IPS - IPS
# IUunique_GO <- setdiff(IPS_GO$GO.ID, uniprot_GO$GO.ID)
# length(unique(IUunique_GO)) # 1789 GO terms unique to IPS
# 
# # Find intersections and unique results for each method (IPS and plob_annot)
# # Cannot compare gene ids because Uniprot has different 'gene ids' because I needed it to pair with Uniprot
# 
# 
# 
...
# plob_annot_GO <- select(plob_annot, gene_id, GO.ID)
# splitted <- strsplit(as.character(plob_annot_GO$GO.ID), ";") #split into multiple GO ids
# plob_annot_GO <- data.frame(v1 = rep.int(plob_annot_GO$gene_id, sapply(splitted, length)), v2 = unlist(splitted)) #list all genes with each of their GO terms in a single row
# colnames(plob_annot_GO) <- c("gene_id", "GO.ID")
# plob_annot_GO <- unique(plob_annot_GO)
# plob_annot_GO$GO.ID <- gsub("F:", "", plob_annot_GO$GO.ID)
# plob_annot_GO$GO.ID <- gsub("C:", "", plob_annot_GO$GO.ID)
# plob_annot_GO$GO.ID <- gsub("P:", "", plob_annot_GO$GO.ID)
# plob_annot_GO$GO.ID <- gsub(" ", "", plob_annot_GO$GO.ID)
# plob_annot_GO <- filter(plob_annot_GO, grepl("GO",GO.ID)) #Genes with GO terms - 1224
# plob_annot_GO <- unique(plob_annot_GO)
# colnames(plob_annot_GO) <- c("gene_id", "GO.ID")
# 
# # Intersection between GO terms for acerv_annot (b2g + uniprot) and IPS
# PI_GO <- intersect(plob_annot_GO$GO.ID, IPS_GO$GO.ID)
# length(unique(PI_GO)) # 97 similar GO terms between acerv_annot (b2g + uniprot) and IPS
# 
# # Difference in GO terms for acerv_annot (b2g + uniprot) and IPS - acerv_annot (b2g + uniprot)
# PIunique_GO <- setdiff(plob_annot_GO$GO.ID, IPS_GO$GO.ID)
# length(unique(PIunique_GO)) # 518 GO terms unique to Uniprot
# 
# # Difference in GO terms for acerv_annot (b2g + uniprot) and IPS - IPS
# IPunique_GO <- setdiff(IPS_GO$GO.ID, plob_annot_GO$GO.ID)
# length(unique(IPunique_GO)) # 1898 GO terms unique to IPS
# 
# ## gene id
# # Intersection between gene_id for pdam_annot (b2g + uniprot) and IPS
# PI_gene <- intersect(plob_annot_GO$gene_id, IPS_GO$gene_id)
# length(unique(PI_gene)) # 2320 similar gene ids between acerv_annot (b2g + uniprot) and IPS
# 
# # Difference in gene_id terms for acerv_annot (b2g + uniprot) and IPS - acerv_annot (b2g + uniprot)
# PIunique_gene <- setdiff(plob_annot_GO$gene_id, IPS_GO$gene_id)
# length(unique(PIunique_gene)) # 1995 gene ids terms unique to Uniprot
# 
# # Difference in gene ids for acerv_annot (b2g + uniprot) and IPS - IPS
# IPunique_gene <- setdiff(IPS_GO$gene_id, plob_annot_GO$gene_id)
# length(unique(IPunique_gene)) # 13248 GO terms unique to IPS
# 
# 
# 
# 
# 
# ## Merge Annotations (again) - merge IPS and plob_annot
# 
...
# IPS_results <- read.csv("~/Desktop/PutnamLab/FunctionalAnnotation/InterProScan/plob.interpro.gff3", header = F, sep = "\t")
...
# length(unique(IPS_results_GO$seqid)) # 15568
# 
...
# 
# # aggregate IPS cols before merging w/ plob_annot
...
# nrow(merge_all_IPS) # 15568
# 
# # merge all info 
# plob_annot_bu <- select(plob_annot, c("top_hit", "gene_id", "uniprotkb_entry", "protein_names", "gene_names", "organism", "ko", "kegg", "GO.ID", "GO_terms"))
# nrow(plob_annot_bu) # 4315
# full_annot <- merge(plob_annot_bu, merge_all_IPS, by = "gene_id", all.x = T)
# write.csv(full_annot, "~/Desktop/plob_FuncAnn_UniP_B2G_IPS.csv")
